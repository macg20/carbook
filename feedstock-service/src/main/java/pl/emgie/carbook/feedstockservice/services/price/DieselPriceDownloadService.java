...
@Service
@Qualifier("dieselPriceService")
public class DieselPriceDownloadService extends BaseService implements FeedstockPriceDownloadService {

    private String downloadPageUrl;

    public DieselPriceDownloadService(@Value("${feedstock.fuel.page.url}") String downloadPageUrl) {
    ...
    public BigDecimal getPrice() {
        ...
        try {
           ...
           return new BigDecimal(StringUtils.replaceCommaToDot(rows.get(1).select("td").get(2).text()));
        } catch (Exception e) {
